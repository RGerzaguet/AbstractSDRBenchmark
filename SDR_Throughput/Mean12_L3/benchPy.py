...
def windowMean(x,y):
    for i in range(12,1024):
        y[i] = (x[i]+x[i-1]+x[i-2]+x[i-3]+x[i-4]+x[i-5]+x[i-6]+x[i-7]+x[i-8]+x[i-9]+x[i-10]+x[i-11])/12.0
...
def benchmark_rx_rate(usrp, rx_streamer, timer_elapsed_event, rx_statistics,flag,recv_buffer,fft_buffer,mean_buffer,abs_buffer,fft_plan):
    ...
    while not timer_elapsed_event.is_set():
        try:
            ...
            num_abs2(fft_buffer,abs_buffer)
            windowMean(abs_buffer, mean_buffer)
...
def main():
    ...
    f = open("../data_benchmark_python_mean12_opti.txt", "w")
