...
def windowMean(x):
    ...
    return (cumsum[16:] - cumsum[:-16]) / 16

def benchmark_rx_rate(usrp, rx_streamer, timer_elapsed_event, rx_statistics,flag,recv_buffer,fft_buffer,fft_plan):
    ...
    while not timer_elapsed_event.is_set():
        try:
            ...
            abs_buffer =  num_abs2(fft_buffer)           
            mean_buffer = windowMean(abs_buffer)
...
def main():
    ...
    f = open("../data_benchmark_python_mean16_L0.txt", "w")
        ...
        for mt in range(1,montecarlo_num+1):
            ...
            rx_thread = threading.Thread(target=benchmark_rx_rate,
                                        args=(usrp, rx_streamer, quit_event,
                                            rx_statistics,flag,recv_buffer,fft_buffer,fft_plan))
