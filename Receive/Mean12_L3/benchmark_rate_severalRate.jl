...
function benchmark(radio,samplingRate,duration)
	...
	while true
		...
		@inbounds @avx for i in 12:nbSamples
			mean_out[i] = (abs_out[i]+abs_out[i-1]+abs_out[i-2]+abs_out[i-3]+abs_out[i-4]+abs_out[i-5]+abs_out[i-6]+abs_out[i-7]+abs_out[i-8]+abs_out[i-9]+abs_out[i-10]+abs_out[i-11])/12.0
		end
...
function main(sdr,duration;args="")
    ...
    # File
    myFile = open("data_benchmark_julia_mean12_opti.txt", "w")
